...
interface BentoCardProps {
  ...
  /** Current col-span — controlled by BentoGrid */
  currentSpanW: number;
  /** Current row-span — controlled by BentoGrid */
  currentSpanH: number;
  /** Callback so BentoGrid can update the grid-item wrapper's span classes */
  onSpanChange: (w: number, h: number) => void;
}
...
export default function BentoCard({
  ...
  const clampSpan = (w: number, h: number) => ({
  ...
  const handleResizeStart = useCallback(
    (e: React.MouseEvent | React.TouchEvent, corner: string) => {
      e.stopPropagation();
      e.preventDefault();
      if (!gridCellWidth || !gridCellHeight) return;
      ...
      resizeRef.current = {
        startX: clientX,
        startY: clientY,
        // Snapshot the span at the moment dragging begins
        startSpanW: currentSpanW,
        startSpanH: currentSpanH,
        corner,
        cellW: gridCellWidth,
      ...
      const handleMove = (ev: MouseEvent | TouchEvent) => {
        ...
        // Snap: how many full cell-steps has the pointer moved?
        const cellDx = Math.round(dx / resizeRef.current.cellW);
        const cellDy = Math.round(dy / resizeRef.current.cellH);

        let newW = resizeRef.current.startSpanW;
        let newH = resizeRef.current.startSpanH;

        if (corner.includes('right')) newW += cellDx;
        if (corner.includes('left')) newW -= cellDx;
        if (corner.includes('bottom')) newH += cellDy;
        if (corner.includes('top')) newH -= cellDy;

        const clamped = clampSpan(newW, newH);
        onSpanChange(clamped.w, clamped.h);
      };
    ...
    [currentSpanW, currentSpanH, gridCellWidth, gridCellHeight, onSpanChange],
  );
  ...
  return (
    // This div fills the grid-item wrapper (w-full h-full).
    // Span/size changes are applied to the wrapper in BentoGrid, not here.
    <div
      ref={cardRef}
        ...
        >
          ...
          <div
            ...
            style={{
              height: '50%',
              background:
                'linear-gradient(to top, rgba(20,18,15,0.95) 0%, rgba(20,18,15,0.6) 50%, transparent 100%)',
